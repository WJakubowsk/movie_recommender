...
class MoviesDatasetPreprocessor:
    ...
    def split_title_column(self):
        self.df["year"] = self.df["title"].apply(
            lambda x: re.findall("\([0-9]{4}\)", x)[0][1:-1]
            if re.findall("\([0-9]{4}\)", x) != []
            else ""
        )
    ...
    def split_genres_column(self):
        ...
        self.df[[f"category_{i}" for i in range(1, max_categories)]] = pd.DataFrame(
            self.df["genres"].str.split("|").tolist(),
            columns=[f"category_{i}" for i in range(1, max_categories)],
        )
        self.df.drop("genres", axis=1, inplace=True)

        for i in range(1, max_categories):
            self.df[f"category_{i}"] = self.df[f"category_{i}"].fillna("")
        self.df["category_1"] = self.df["category_1"].str.replace(
            "\(no genres listed\)", ""
        )

    def train_test_split(self):
        self.df["timestamp"] = pd.to_datetime(self.df["timestamp"], utc=True)
        self.df["rank"] = self.df.groupby(["userId"])["timestamp"].rank(
